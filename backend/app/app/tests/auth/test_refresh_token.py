...
async def test_verify_token(client: AsyncClient, create_auth_pair: TokenPair):
    ...
    response = await client.get("/api/v1/token/", headers={"Authorization": f"Bearer {tokens.access}"})
...
async def test_update_token(client: AsyncClient, create_auth_pair: TokenPair):
    tokens = create_auth_pair
    response = await client.post("/api/v1/token/refresh/", json={"value": tokens.refresh})
    assert response.status_code == int(HTTPStatus.OK)
...
@pytest.mark.anyio
async def test_not_in_db_token(client: AsyncClient, create_not_in_db_refresh_token: TokenBase):
